...
export class DB {
    ...
    private async execute(sql: string, params: any[] = []): Promise<any[]> {
        ...
        if (this.db != null) {
            try {
                ...
                await (await this.db).withTransactionAsync(async () => {
                    if (sql.trim().toUpperCase().startsWith('SELECT')) {
                        resultRows = await this.db.getAllAsync(sql, params);
                    } else {
            ...
            throw new DBException('db is null');
        }
    }
    ...
    async addNewTable(tableName: string, rows: RowType[]): Promise<boolean> {
        ...
        if (exist) throw new DBException('Table with this name already exists');
    ...
    async getDataFromTable(tableName: string) {
        const exist = await this.isTableExists(tableName);
        if (!exist) throw new DBException('table not exist');
        return this.get(`SELECT * FROM ${tableName};`);
    }

    async setDataToTable(tableName: string, data: MoneyType): Promise<boolean> {
        if (!(await this.isTableExists(tableName))) throw new DBException('table not exist');

        const sql = `INSERT INTO ${tableName} (money, time_data, comment) VALUES (?, ?, ?);`;
        return this.set(sql, [data.money, data.time_data, data.comment]);
    }
